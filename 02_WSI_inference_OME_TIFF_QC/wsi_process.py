...
def slide_process_single(model, tis_det_map_mpp, slide, patch_n_w_l0, patch_n_h_l0, p_s, m_p_s, colors,
    ...
    for he in tqdm(range(patch_n_h_l0), total=patch_n_h_l0):
        ...
        for wi in range(patch_n_w_l0):
            ...
            #
            if np.count_nonzero(td_patch == 0) > 5:  #TODO #what is 50 here? # here change to check of segmentation map # if greater than 50 px of signal
                # Generate patch
    ...
    end_image_1class = end_image_1class.resize((patch_n_w_l0*200, patch_n_h_l0*200), Image.Resampling.LANCZOS) 
    #
